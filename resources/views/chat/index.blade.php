...
<!-- Chat Container -->
<div class="chat-container" style="display: flex; gap: 20px; height: calc(100vh - 160px);">
    <!-- User List -->
    <div class="chat-users" style="width: 20%; background-color: #fff; border: 1px solid #ddd; padding: 10px; overflow-y: auto;">
    <form method="GET" action="{{ route('chat.index') }}" style="display: flex; align-items: center; gap: 5px; margin-bottom: 10px;">
    <input type="text" name="search" placeholder="Search user by ID" value="{{ request()->input('search') }}" 
    ...
    <button type="submit" 
        ...
        <h4 style="margin-top: 10px;">Inbox</h4>
<ul style="list-style: none; padding-left: 0;">
    ...
    <!-- Chat Box -->
    <div class="chat-box" style="flex: 1; background-color: #fff; border: 1px solid #ddd; padding: 20px; overflow-y: auto;">
        @if ($receiver)
            <div style="font-size: 20px; font-weight: bold; margin-bottom: 15px;">
                Chat with {{ $receiver->name }}
            </div>
            <div style="height: calc(100% - 120px); overflow-y: scroll; background-color: #f9f9f9; padding: 10px;">
    @foreach ($messages as $message)
        <div style="margin-bottom: 10px; {{ $message->sender_id == auth()->id() ? 'text-align: right;' : 'text-align: left;' }}">
            <strong style="color: {{ $message->sender_id == auth()->id() ? '#1e88e5' : '#43a047' }};">
                {{ $message->sender_id == auth()->id() ? 'You' : $receiver->name }}
            </strong>
    ...
    <textarea name="message" placeholder="Type a message..." required 
              style="flex: 1; height: 50px; padding: 5px; border: 1px solid #ccc; border-radius: 5px;"></textarea>
    <button type="submit" 
            style="height: 50px; padding: 0 20px; background-color: #007bff; color: white; border: none; border-radius: 5px; cursor: pointer;">
        Send
    </button>
</form>

        @else
            <p style="text-align: center; font-size: 18px; color: #555;">Select a user to start chatting!</p>
        @endif
    </div>
</div>
